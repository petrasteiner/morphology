...
<GNcompoundswithCELEXRDF> Takes xml-files of GermaNet and extracts trees structures of the compounds.  Many options can be made, e.g. restricting the trees to a certain depth of analysis. Optionally, these analyses can be combined with the analyses of the refurbished CELEX database for German. Also here, many options can be made, e.g. skip conversions or restrict the trees to a certain depth of analysis.
...
=head1 SYNOPSIS
Synopsis in GNcompoundswithCELEXRDF.help

=head1 REQUIREMENTS

files GMOLoutputneworthography
      GSOLoutputneworthographyinputdirectory with GermaNet xml files
or inputfile (GN xml file)
=head1 USE

Description in GNcompoundswithCELEXRDF.help

=head1 ARGUMENTS

see  GNcompoundswithCELEXRDF.help
...
Distribution only by Petra Steiner, CC-BY-SA4.0, only for peaceful purposes 
...
#put the GN files into -d DIRECTORY, e.g. filesGN17
#perl GNcompoundswithCELEX.pl -d filesGN17 -rmpn -rmfw -addfl -n 4 -celex > out2402 2>&1 &

# as above plus add linguistic information (currently just the constructs)
#perl GNcompoundswithCELEX.pl -d filesGN17 -rmpn -rmfw -addfl -n 4 -celex -it > out2402 2>&1 &

# perl GNcompoundswithCELEX.pl -d filesGN17 -rmpn -rmfw -addfl -n 4 -celex -pos -it -ctags -zn 3 -par -levperc 0.5 > out2006 2>&1
...
chomp $cwd;
$sInputFilename = "$cwd\/filesGN17\/nomen.Artefakt.xml";
...
#		    print "Tags in compound: @tagnames\n";
			...
			if ($nodename eq "modifier")
			{
				...
				{
				    print "Non-accepted compound entry (Affixoid or opaque): $orthform with property $pos for $form\n";
				}
			    }
...
if ($flagzusammenrueckungen)
{    
  %  $hashfilecelexlistinverted =  $cwd . "\/". $hashfilecelexlistinverted; # because of flagcelex
}
